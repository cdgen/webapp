  ...
  return (
    <div className={classes}>
      {props.items.map((item, idx) => {
        ...
        const onClick = props.onClick(idx);
        
        return (
          <div key={idx} style={styles} className={classesOption} onMouseOver={onHover} onClick={onClick}>
  ...
  render() {
    ...
    return (
      <div className="backgroundSlider">
        <VisualSlider {...state} />
        <Options {...state} items={ITEMS} onHover={this._boundOnIndexChange} onClick={this._boundOnClick} active={this.state.current}/>
      </div>
    );
  }
}
